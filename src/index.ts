...
const initWorker = async (settings: IndexTypes.BlockchainSettings): Promise<Worker> => {
    const dfd: IndexTypes.Deferred<Worker> = createDeferred(-1);
...
class BlockchainLink extends EventEmitter {
    ...
    __send: <R>(message: any) => Promise<R> = async (message) => {
        ...
        const dfd: IndexTypes.Deferred<any> = createDeferred(this.messageId);
